    ...
    for (let i in this.dataNotification) {
      if (
        ...
        for (let j in this.sensors_data_last) {
          if (this.sensors_data_last[j].device_mac == this.dataNotification[i].device_mac) {
            if(parseFloat(this.sensors_data_last[j].temperature) > parseFloat(this.dataNotification[i].notification_value)){
            try {
        ...
        for (let j in this.sensors_data_last) {
          if (this.sensors_data_last[j].device_mac == this.dataNotification[i].device_mac) {
            if(parseFloat(this.sensors_data_last[j].humidity) > parseFloat(this.dataNotification[i].notification_value)){
              try {
              await this.platform.ready();
              const permission = await this.notification.requestPermission();
        ...
        for (let j in this.sensors_data_last) {
          if (this.sensors_data_last[j].device_mac == this.dataNotification[i].device_mac) {
            if(parseFloat(this.sensors_data_last[j].pressure) > parseFloat(this.dataNotification[i].notification_value)){
              try {
              await this.platform.ready();
              const permission = await this.notification.requestPermission();
